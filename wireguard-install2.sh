...
function installQuestions() {
	...
	until [[ ${SERVER_WG_NIC} =~ ^[a-zA-Z0-9_]+$ && ${#SERVER_WG_NIC} -lt 16 ]]; do
		read -rp "WireGuard interface name: " -e -i wg0b SERVER_WG_NIC
	done
...
ALLOWED_IPS=${ALLOWED_IPS}" >/etc/wireguard/params2
...
# Check if WireGuard is already installed and load params2
if [[ -e /etc/wireguard/params2 ]]; then
	source /etc/wireguard/params2
	manageMenu
else
	installWireGuard
fi
